...
function formatInfo(obj) {
  ...
  const androidVersion = obj.android_version;
  const buildId = obj.android || obj.buildId || obj.build_id;
  const sdk = obj.sdk_int || obj.sdk || obj.sdkInt;

  const physical = obj.physical ?? obj.is_physical_device;
  ...
  const cwd = obj.cwd;
  const total = obj.storage_total_gb;
  const free = obj.storage_free_gb;

  if (androidVersion)
    lines.push(`🤖 Android: ${androidVersion}${sdk ? ` (SDK ${sdk})` : ""}`);
  ...
  if (physical !== undefined) lines.push(`📍 Physical: ${physical}`);
  if (cwd) lines.push(`📂 CWD: ${cwd}`);

  if (total !== undefined || free !== undefined) {
    lines.push(`💾 Storage: ${total ?? "?"} GB total, ${free ?? "?"} GB free`);
  }
