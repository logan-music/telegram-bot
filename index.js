...
bot.onText(/^\/help$/i, (msg) => {
  const text = [
    ...
    "/send <path> — upload file via device and get public URL",
    "/rm <path> — remove file",
...
bot.onText(/^\/upload\s+(.+)$/i, async (msg, m) => {
  ...
  try {
    ...
    if (!prepPayload) {
      await bot.sendMessage(chatId, "❌ prepare_upload failed (no payload)");
      return;
    }
    ...
    await bot.sendMessage(chatId, "☁️ Uploading to Supabase Storage…");
    ...
    if (data && data.error) {
      await bot.sendMessage(chatId, `❌ Upload failed (function): ${data.error} ${data.detail ? ` - ${JSON.stringify(data.detail)}` : ""}`);
      return;
    }

    await bot.sendMessage(chatId, `✅ Upload complete\nBucket: ${data.bucket}\nPath: ${data.path}\nSize: ${data.size ?? "unknown"}`);
  } catch (e) {
    await bot.sendMessage(chatId, `❌ upload error: ${e.message || e}`);
  }
});

/* ===== /send (upload via device and return public URL) ===== */
bot.onText(/^\/send\s+(.+)$/i, async (msg, m) => {
  ...
  try {
    ...
    // success detection (support different shapes)
    const ok = (payload.success === true) || (payload.upload && payload.upload.success === true) || (payload.result && payload.result.success === true);

    if (!ok) {
      await bot.sendMessage(chatId, `❌ send failed: ${JSON.stringify(payload || res || {})}`);
      return;
    }

    // Try to detect actual bucket/dest from payload (fallback to what we sent)
    const actualBucket = payload.bucket || payload.upload?.bucket || bucket;
    const actualDest = payload.dest || payload.upload?.path || payload.upload?.key || payload.path || dest;

    // generate public URL using Supabase JS SDK
    try {
      const { data: urlData, error: urlErr } = supabase.storage.from(actualBucket).getPublicUrl(actualDest);
      if (urlErr) {
        await bot.sendMessage(chatId, `✅ File uploaded\n📁 ${actualDest}\n(But failed to get public URL: ${urlErr.message || JSON.stringify(urlErr)})`);
        return;
      }
      ...
      if (publicUrl) {
        ...
        await bot.sendMessage(chatId, `✅ File uploaded\n📁 ${actualDest}\n(Upload OK but public URL not available)`);
      }
    } catch (e) {
      await bot.sendMessage(chatId, `✅ File uploaded\n📁 ${actualDest}\n(Uploaded but couldn't get public URL: ${e.message || e})`);
    }
  } catch (e) {
    await bot.sendMessage(chatId, `❌ send error: ${e.message || e}`);
  }
});
...
bot.onText(/^\/rm\s+(.+)$/i, async (msg, m) => {
  ...
  await bot.sendMessage(chatId, `🗑️ Removing file: ${path}`);
  try {
    const cmd = await sendCommand(deviceId, "delete_file", { path });
    const res = await waitForResultRealtime(cmd.id, 30_000);
    const payload = unwrapResult(res);
    if (payload && payload.success) await bot.sendMessage(chatId, `✅ Removed ${path}`);
    else await bot.sendMessage(chatId, `❌ remove failed: ${JSON.stringify(payload || res || {})}`);
  } catch (e) {
    await bot.sendMessage(chatId, `❌ rm error: ${e.message || e}`);
  }
});
...
bot.onText(/^\/rd\s+(.+)$/i, async (msg, m) => {
  ...
  await bot.sendMessage(chatId, `🗑️ Removing directory (recursively): ${path}`);
  try {
    const cmd = await sendCommand(deviceId, "delete_dir", { path });
    const res = await waitForResultRealtime(cmd.id, 60_000);
    const payload = unwrapResult(res);
    if (payload && payload.success) await bot.sendMessage(chatId, `✅ Removed directory ${path}`);
    else await bot.sendMessage(chatId, `❌ rd failed: ${JSON.stringify(payload || res || {})}`);
  } catch (e) {
    await bot.sendMessage(chatId, `❌ rd error: ${e.message || e}`);
  }
});
...
bot.onText(/^\/ping$/i, async (msg) => {
  ...
  try {
    ...
    const payload = unwrapResult(res) ?? {};
    if (payload && payload.ts) await bot.sendMessage(msg.chat.id, `🏓 Pong — ts: ${payload.ts}`);
    else await bot.sendMessage(msg.chat.id, "🏓 Pong");
  } catch (e) {
