...
/**
 * formatInfo tolerant to multiple field names returned by app
 * Accepts fields (any): android, android_version, buildId, physical, is_physical_device, platform, cwd, manufacturer, model, device, product, storage_total_gb, storage_free_gb, sdk_int
 */
function formatInfoFull(obj) {
  if (!obj || typeof obj !== "object") return "";

  // unwrap success/data wrapper
  if (obj.success === true && typeof obj === "object") {
    obj = obj.data || obj;
  }
  ...
  // Core Android info
  append("🤖 Android Version", obj.android_version || obj.android || obj.buildId || obj.build_id);
  append("SDK", obj.sdk_int || obj.sdk || obj.sdkInt);
  append("Platform", obj.platform || "android");
  append("Physical Device", obj.physical ?? obj.is_physical_device);

  // Device identity
  append("🏭 Manufacturer", obj.manufacturer);
  append("📱 Model", obj.model);
  append("🔧 Device", obj.device);
  append("📦 Product", obj.product);
  append("🧩 Build ID", obj.buildId || obj.build_id);

  // Runtime / Environment
  append("📂 CWD", obj.cwd);
  append("💾 Storage Total (GB)", obj.storage_total_gb);
  append("💾 Storage Free (GB)", obj.storage_free_gb);

  // Optional fields: fallback to metadata if present
  ...
  // Timestamp
  append("⏱️ Timestamp", obj.timestamp || obj.ts || new Date().toISOString());
...
function formatListingPlain(result) {
  ...
  if (Array.isArray(result.folders) || Array.isArray(result.files)) {
    const folders = Array.isArray(result.folders)
      ? result.folders.map(f => `📁 ${safeString(f)}`)
      : [];

    const files = Array.isArray(result.files)
      ? result.files.map(f => safeString(f))
      : [];
...
bot.onText(/^\/info$/i, async (msg) => {
  ...
  try {
    ...
    const timeout = online ? 20_000 : SEVEN_DAYS_MS;
    if (!online) await bot.sendMessage(msg.chat.id, `⚠️ Device offline — info queued. I will wait (up to 7 days).`);
