...
function formatInfo(obj) {
  ...
  // accommodate multiple possible keys
  const android = obj.android_version || obj.android || obj.buildId || obj.build_id || obj.build_fingerprint;
  const sdk = obj.sdk_int || obj.sdk || obj.sdkInt;
  const physical = obj.physical ?? obj.is_physical_device ?? obj.isPhysical;
  const manufacturer = obj.manufacturer || obj.mfg;
  const model = obj.model || obj.device_model;
  const device = obj.device || obj.device_name;
  const product = obj.product;
  const platform = obj.platform || obj.platform_name;
  ...
  if (android) lines.push(`🤖 Android: ${safeString(android)}${sdk ? ` (SDK ${safeString(sdk)})` : ""}`);
  ...
  if (physical !== undefined) lines.push(`📍 Physical: ${safeString(physical)}`);
  if (cwd) lines.push(`📂 CWD: ${safeString(cwd)}`);
  if (total !== undefined || free !== undefined) {
    lines.push(`💾 Storage: total=${safeString(total ?? "n/a")} GB free=${safeString(free ?? "n/a")} GB`);
  }

  // fallback: if nothing, try to print keys
  if (!lines.length) {
...
function formatListingPlain(result, requestedPath = "") {
  ...
  if (Array.isArray(result.entries)) {
    ...
    for (const e of result.entries) {
      ...
      else files.push(`📄 ${name}${e.size ? ` (${formatBytes(e.size)})` : ""}`);
    }
...
bot.onText(/^\/rm\s+(.+)$/i, async (msg, m) => {
  ...
  try {
    ...
    const timeout = online ? 30_000 : SEVEN_DAYS_MS;
    if (!online) await bot.sendMessage(chatId, `⚠️ Device offline — delete queued. I will wait for result (up to 7 days).`);
