...
bot.onText(/^\/upload\s+(.+)$/i, async (msg, m) => {
  ...
  try {
    ...
    if (prepPayload) {
      source = {
        ...
        file_path: prepPayload.path || prepPayload.file_path || path,
        meta: prepPayload,
      };
      fileName = prepPayload.name || fileName;
    }
  } catch (e) {
  ...
  /* =========================
     2️⃣ FALLBACK: read_file
     ========================= */
  if (!source) {
    bot.sendMessage(chatId, "📄 Reading file content…");

    const readCmd = await sendCommand(deviceId, "read_file", {
      path,
      encoding: "base64",
      max_size: 25 * 1024 * 1024, // 25MB safety
    });

    const readRes = await waitForResultRealtime(readCmd.id, 120_000);
    const readPayload = unwrapResult(readRes);

    if (!readPayload?.content) {
      bot.sendMessage(chatId, "❌ Failed to read file");
      return;
    }
    ...
    fileName = readPayload.name || fileName;
  }

  /* =========================
     3️⃣ Upload to Supabase
     ========================= */
  const destName = fileName.replace(/\s+/g, "_");
  const payload = {
    device_id: deviceId,
    bucket: "mydrive",
  ...
  try {
    ...
    if (data?.error) {
      bot.sendMessage(chatId, `❌ Upload failed: ${data.error}`);
      return;
    }
